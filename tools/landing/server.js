...
async function serveStatic(req, res) {
  ...
  } else if (pathname === "/landing" || pathname === "/landing/") {
    filePath = path.join(publicDir, "index.html");
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "GET" && req.url === "/api/auth-status") {
    sendJson(res, 200, { locked: !skipAuth && Boolean(accessPassword), authorized: isAuthorized(req) });
    return;
  }
...
server.listen(port, host, () => {
  if (process.stdout?.writable) {
    process.stdout.write(`KV portal running at http://127.0.0.1:${port} (landing at /index.html)\n`);
  }
});
