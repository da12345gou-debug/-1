...
if (emptyState) {
  emptyState.innerHTML = `
    <div class="bottle-loader" aria-label="等待生成" role="img">
      <span class="bottle-cap"></span>
      <span class="bottle-body">
        <span class="bottle-label"></span>
...
if (emptyState && !emptyState.querySelector(".empty-copy")) {
  const copy = document.createElement("p");
  copy.className = "empty-copy";
  copy.textContent = "生成完成后，结果会出现在这里。";
  emptyState.append(copy);
}

function setPreviewLoading(isLoading, text = "") {
  if (!emptyState) return;
  if (isLoading) clearMainDownload();
  emptyState.classList.toggle("is-generating", isLoading);
  emptyState.dataset.state = isLoading ? "loading" : "idle";
  const copy = emptyState.querySelector(".empty-copy");
  if (copy) copy.textContent = isLoading ? text : "生成完成后，结果会出现在这里。";
  if (isLoading && !resultList.contains(emptyState)) {
    resultList.innerHTML = "";
    resultList.append(emptyState);
  }
}

function clearMainDownload() {
...
function toFriendlyError(messageText) {
  ...
  if (/timeout|Time-out|524|504|超时/i.test(text)) return "生成等待时间过长，请稍后重试。";
  if (/fetch|network|Failed to fetch/i.test(text)) return "网络连接失败，请刷新页面后重试。";
  if (/policy|violated/i.test(text)) return "图片生成被拦截，请换一张参考图或调整描述后再试。";
  if (/API|Base URL|JSON|HTML/i.test(text)) return "接口返回异常，请检查 API 配置或刷新后重试。";
...
function startGenerationTimer(segmentCount) {
  ...
  function tick() {
    ...
    const previewText = remainingSeconds > 0
      ? `预计剩余 ${formatDuration(remainingSeconds)}`
      : `已超过预计 ${formatDuration(Math.abs(remainingSeconds))}`;
    setPreviewLoading(true, previewText);
...
async function previewFile(input, img, shouldTrackPrototype = false) {
  ...
  img.src = dataUrl;
  input.closest(".drop-zone").classList.add("has-image", "is-selected");
...
function setMainDownload(downloadUrl, titleText) {
  if (!downloadLink || !downloadUrl) return;
  const filename = createDownloadName(titleText || "landing-page", downloadUrl);
...
async function renderResults(heroDataUrl, images, segmentMeta = []) {
  ...
  if (!stitched.tooLarge) {
    ...
    primaryDownload = {
      titleText: "landing-page-section",
  ...
  setMainDownload(primaryDownload.downloadUrl, primaryDownload.titleText);
}
...
robotReferenceInput?.addEventListener("change", async () => {
  ...
  robotReferenceInput.closest(".drop-zone").classList.add("has-image", "is-selected");
});
...
form.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    buttonText.textContent = hasGeneratedResult ? "重新生成" : "生成落地页";
  }
});
