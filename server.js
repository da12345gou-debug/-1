...
机器人造型硬规则：ROBOT_SHAPE_REFERENCE_ONLY 图片是唯一的机器人造型来源，必须严格按照图4/机器人参考图的造型和细节。只允许改变装束、道具、职业和行为，不允许改变机器人本体轮廓、头身比、黑色面屏形状、竖条眼睛、白色外壳、短小侧臂、无手无脚无腿、悬浮状态和底部蓝色悬浮光。禁止把机器人画得更圆润、更胖、更像玩偶、更像其他品牌机器人，禁止长出手、脚、腿、手指、鞋子、圆球身体或额外肢体。若无法保持造型准确，宁可不出现机器人。ROBOT_SHAPE_REFERENCE_ONLY 图片仅用于锁定白色机器人造型和细节，不作为页面内容模块，不参考其背景。${brandSafetyGuide}${segmentGuide}

最终质检硬规则：生成前最后检查所有文字。所有中文、数字和英文都必须使用正常字体宽高比，字形宽度为 100% 正常比例；禁止横向拉长、横向加宽、纵向拉长、上下拉伸、压扁、倾斜、变窄、变形、extended/expanded/condensed 字体效果。不得为了填满标题胶囊、卡片或画布而改变字形比例。文字比例错误比装饰不足更严重。`;
}
