...
- 所有文字必须保持正常字体比例，禁止上下拉伸、横向压扁、纵向压扁、倾斜变形或为了塞进胶囊而改变字形比例；文字放不下时只能加宽标题框或换行，不能拉伸/压缩字体。
...
注意画面中可融入很小的图4的白色机器人形象元素（注意严格按照图4的形象，注意该形象为悬浮，无脚、无腿，手臂必须严格按照参考图的“手臂”，请严格参考，并参考画面的主题和需求给该形象赋予职业和行为，要求每一个出现的机器角色的造型都严格参考参考图，但是装束和行为不同。注意该形象可以出现至多3个，大小要有明确区分，可以产生互动，也可以不出现）
严格按照图4的造型和细节。白色机器人必须保持参考图的头部比例、黑色面屏、两条竖向发光眼、白色悬浮身体和参考图中的手臂结构；每个机器人只能使用参考图中的手臂造型，禁止出现多手臂、四个手臂、额外手臂、手掌、手指、腿、脚或人形四肢。新增的 ROBOT_SHAPE_REFERENCE_ONLY 图片与图4并列，仅用于锁定白色机器人造型和细节，不作为页面内容模块，不参考其背景。${brandSafetyGuide}${segmentGuide}`;
}
